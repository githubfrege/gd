...
class CFR:
    ...
    def cfr(self, h: History, i: Player, pi: List[float]) -> float:
        if self.mynode < 500:
            print(h)
        if 'crrr' in h.__repr__():
            print('break')
        self.mynode = self.mynode + 1
        if h.is_terminal():
        ...
        for t in range(self.epochs):
            ...
            #for I in self.info_sets.values():
                #for a in I.actions():
                    ...
                    # })
                   # print(f'strategy.{I.key}.{a} {I.strategy[a]}')
                    #print(f'average_strategy.{I.key}.{a} {I.strategy[a]}')
                   # print(f'regret.{I.key}.{a} {I.regret[a]}')
                   # print(f'current_regret.{I.key}.{a} {I.current_regret[a]}')
